...
class BirdViewProducer:
    ...
    def produce(
        ...
        # draw actor 
        masks, _ = self._render_actors_masks(agent_bbox_list, 
                                          vehicle_bbox_list, 
    ...
    def _render_actors_masks(
        ...
        masks[BirdViewMasks.AGENT.value], agent_3d_box = self.masks_generator.draw_bbox_mask(
            agent_bbox_list
        )
        masks[BirdViewMasks.VEHICLES.value], vehicle_3d_box = self.masks_generator.draw_bbox_mask(
            vehicle_bbox_list
        )
        masks[BirdViewMasks.PEDESTRIANS.value], pedestrians_3d_box = self.masks_generator.draw_bbox_mask(
            pedestrians_bbox_list
        )
        masks[BirdViewMasks.OBSTACLES.value], obstacle_3d_box = self.masks_generator.draw_bbox_mask(
            obstacle_bbox_list
        )
        
        
        return masks, agent_3d_box+vehicle_3d_box+pedestrians_3d_box+obstacle_3d_box
