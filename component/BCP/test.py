...
def find_tracker(args, basic, variant, tracking, start, end, target_points_dict=None):
    """
    ...
    gx, gy = target_points_dict[basic+'_'+variant][f"{end-1:08d}"]
...
def testing(args, model, test_set, tracking_list, device):
    ...
    if args.use_intention:
        target_points_dict = json.load(open("./utils/goal_list.json"))
    ...
    for cnt, (test_sample, tracking_results) in enumerate(zip(test_set[:], tracking_list), 1):
        ...
        for frame_id in range(first_frame_id, last_frame_id+1):
            ...
            trackers, tracking_id, intention_inputs, target_point = find_tracker(
                args, basic, variant, tracking_results, st, et+1, target_points_dict)
            ...
            for l in range(st, et+1):
                ...
                img = Image.open(camera_path).convert('RGB').copy()
                ...
                if args.use_intention:
                    ...
                    # Bx2  -> Bx(1+N)x128
                    goal_feature = model.target_model(target_point)
